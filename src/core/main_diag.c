...
static void log_crypto_policies_human(struct app_config *cfg, int config_id) {
    fprintf(stderr,
            "[CRYPTO POLICIES] ne_profiles.id=%d — ne_policies as loaded (grouped by profile)\n",
            config_id);

    if (cfg->profile_count <= 0) {
        fprintf(stderr,
                "  (no profiles; policies=%d — check ne_profiles / DB load)\n",
                cfg->policy_count);
...
void main_diag_log_loaded_config(struct app_config *cfg, int config_id) {
    fprintf(stderr,
            "[DB LOAD] ne_profile_id=%d crypto_enabled=%d encrypt_layer=%d "
