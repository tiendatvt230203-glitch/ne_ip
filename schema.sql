...
DO $$ BEGIN
    CREATE TYPE encryption_method_enum AS ENUM (
        'aes-gcm-128', 'aes-gcm-256', 'aes-ctr-128', 'aes-ctr-256'
    );
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
CREATE TABLE IF NOT EXISTS ne_profiles (
    id              SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_encryption_profile_name ON ne_profiles (name);

CREATE TABLE IF NOT EXISTS ne_policies (
    id              INT             PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_encryption_profile_id  ON ne_policies (profile_id);
CREATE INDEX IF NOT EXISTS idx_encryption_priority    ON ne_policies (priority);
CREATE INDEX IF NOT EXISTS idx_encryption_action      ON ne_policies (action);

CREATE TABLE IF NOT EXISTS ne_lan (
...
CREATE TABLE IF NOT EXISTS ne_wan (
    id              UUID            PRIMARY KEY DEFAULT gen_random_uuid(),
    interface       VARCHAR(100)    NULL,
    profile_id      INT             NOT NULL REFERENCES ne_profiles(id) ON DELETE CASCADE,
